...
with st.form("attendance_form"):
    ...
    conducted = st.number_input("Classes Conducted", min_value=1, step=1)
    ...
    if submit:
        add_attendance(subject, conducted, attended)
...
if records:
    df = pd.DataFrame(records, columns=["ID", "Subject", "Conducted", "Attended"])

    df["Attendance %"] = round((df["Attended"] / df["Conducted"]) * 100, 2)
    ...
    if not low_attendance.empty:
        st.warning("⚠ Attendance below 75% detected.")

        st.dataframe(low_attendance, use_container_width=True)

    record_id = st.selectbox("Delete Record", df["ID"])
